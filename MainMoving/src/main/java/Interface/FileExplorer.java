...
public class FileExplorer extends JPanel {
	...
	public FileExplorer(ManageFolder move) {
		...
		this.setSize((int) (width * 0.445), (int) (height * 0.445));
		...
		this.add(getSearchPnl(), BorderLayout.PAGE_START);
		...
		this.add(sidePnl, BorderLayout.LINE_START);
	...
	/*private JPanel createFilePanel(File file) {
		...
		File imageFile = helpAction.getFileLogo(file);
		Image image;
		if(imageFile != null)
			image = helpAction.loadImage(imageFile.getPath());
		else {
		...
		System.out.println(helpAction.fileHaveLogo(file));
		// img.setIcon(icon);
		pnl.add(img, BorderLayout.CENTER);

		FileName text = new FileName(file.getName());

		pnl.add(text, BorderLayout.PAGE_END /* BorderLayout.LINE_END */    /*);
	...
	public class FilePanel extends FocusContainer implements FocusListener {
	...
	class FileName extends JTextField implements FocusListener {
